...
public class ModelReservationToGoogleTest
{
    private readonly MappingHandler _mappingHandler;
    public ModelReservationToGoogleTest()
    {
    ...
    public async Task Should_Map_Successfully_When_Data_Is_Valid()
    {
        ...
        model.Should().BeOfType<GoogleReservation>();

        if(model is GoogleReservation googleReservation)
        {
            googleReservation.HotelName.Should().Be("Grand Berlin Hotel");
            googleReservation.StayDays.Should().Be(5);
        }
    }
