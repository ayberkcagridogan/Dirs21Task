...
public class GoogleReservationToModelTest
{
    private readonly MappingHandler _mappingHandler;
    public GoogleReservationToModelTest()
    {
    ...
    public async Task Should_Map_Successfully_When_Data_Is_Valid()
    {
        ...
        //  Act
        var result = await _mappingHandler.Map(data , "Google.Reservation", "Model.Reservation", cts.Token);

        // Assert  
        result.IsSuccess.Should().BeTrue();
        
        var model = result.Value;
        model.Should().BeOfType<ModelReservation>();

        if(model is ModelReservation modelReservation)
        {
            modelReservation.HotelName.Should().Be("Grand Berlin Hotel");
            modelReservation.RoomType.Should().Be(RoomType.Deluxe);
        }
    }
    ...
    public async Task Should_Map_Error_When_Invalid_Enum()
    {
        ...
        result.Errors.Select(e => e.Message).Should().Contain("Invalid value 'InvalidRoom' for enum RoomType");
    }
    ...
    public async Task Should_Map_Error_When_Missing_Property()
    {
        ...
        // Assert  
        result.IsFailed.Should().BeTrue();
        result.Errors.Select(e => e.Message).Should().Contain("'Hotel Name' must not be empty.");
    }
